$(function () {
  ...
  $(document).on('click', 'input[value=Play]', function () {
    ...
    $.ajax(projectRequest).done(function (data) {
      ...
      $('.tasks_container').append(`<button class="list-group-item list-group-item-action active" id="task_header" type="button">${data.name}</button>`)
      // $('#tasks_header').html(data.name);
      for (const task of data.tasks) {
        $('.tasks_container').append(`<button type="button" class="list-group-item list-group-item-action task_button" task-id='${task.id}'>${task.title}</button>`);
      }
    });
  });
